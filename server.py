...
# V8.5 deployment hardening:
# GitHub's web uploader can flatten folders. Reconstruct the expected package
# layout before importing tools/agent/core/engines.
from bootstrap_layout import ensure_layout
BOOTSTRAP_INFO = ensure_layout()
...
from agent.deepseek_client import configured as deepseek_configured, model_name as deepseek_model, classify_intent, extract_supply_chain as deepseek_extract_supply_chain, general_guidance, analyze_tool_result, connection_test as deepseek_connection_test
...
app=FastAPI(title='WaterPulse AI Agent API', version='8.5')
...
@app.get('/api/health')
def health():
    return {'status':'ok','app':'WaterPulse AI Agent','version':'8.5','deployment_layout':'flat-upload-safe','deepseek_configured':deepseek_configured(),'deepseek_model':deepseek_model(),'field_contract':contract_summary(),'timestamp':now_iso()}
...
def _auto_analyze(sid: str, sess: dict[str, Any], *, requested_scenario: str|None=None,
    ...
    trace.append(_trace('字段合同检查', governance.get('status','unknown'), '已载入 E 组 field_dictionary 与 field_gap_register；仅用于字段必填性、来源和缺口治理，不覆盖冻结模型公式。'))
    if governance.get('missing_fields'):
        return {'status':'insufficient','message':'要继续正式分析，还缺少企业侧关键字段：'+ '；'.join(governance.get('missing_details') or governance.get('missing_fields') or []) + '\n\n' + user_requirements_text(),
...
def agent_status():
    return {'status':'ok','agent_version':'8.4-chat-deepseek-field-governance',
...
async def agent_message(
    ...
    if intent in {'data_help','general','explain'} and msg:
        fallback=user_requirements_text()
        ans,meta=general_guidance(msg,fallback,deepseek_context('baseline'))
...
def data_contract():
    return {'status':'success','summary':contract_summary(),'requirements':user_requirements_text(),'context':deepseek_context('baseline')}
