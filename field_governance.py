...
def user_requirements_text() -> str:
    ...
    lines = [
        '做正式 Baseline，你主要需要提供 3 类企业私有数据：',
        '1）原材料；2）供应节点/地区；3）各节点采购数量或采购份额。',
        '建议同时提供企业名称、采购年份/参考期和采购口径（数量/金额）。',
        '如果只覆盖部分采购来源，Unknown 份额会被保留，不会把已知节点自动归一化到 100%。',
        '地点标准化、WS/SV/DR、材料参数和证据元数据由后台注册数据层补充；缺失或未批准时系统会返回 partial/insufficient，而不是让 AI 猜值。',
    ]
    if wgap:
        lines.append('字段合同提示：采购权重 W 属于企业私有字段，当前没有公共数据源可替代，缺失时正式 Baseline 会停止。')
    return '\n'.join(lines)
