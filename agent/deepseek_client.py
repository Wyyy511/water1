...
def connection_test() -> dict[str, Any]:
    """Make a minimal live API call. The key is read only from DEEPSEEK_API_KEY."""
    if not configured():
        return {"ok": False, "configured": False, "model": None, "message": "DEEPSEEK_API_KEY is not configured"}
    try:
        text = _post([
            {"role": "system", "content": "You are a connection test. Reply with exactly OK."},
            {"role": "user", "content": "ping"},
        ], temperature=0.0, max_tokens=8)
        return {"ok": True, "configured": True, "model": model_name(), "reply": text[:50]}
    except Exception as exc:
        return {"ok": False, "configured": True, "model": model_name(), "message": str(exc)[:500]}
